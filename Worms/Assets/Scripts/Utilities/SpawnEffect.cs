...
namespace Utilities
{
    ...
    public class SpawnEffect : MonoBehaviour
    {
        ...
        private GameObject _activatedGameObject;
        ...
        /// </summary>
        private float _t = 0f;
        private void ScaleUp(Vector3 startPos, Vector3 endPos, Vector3 endScale, float duration = 0.25f)
        {
            _t += Time.deltaTime / duration;
            _t = Mathf.Min(_t, 1f);

            //float t = EasingUtils.Bounce.Out(_t);
            float t = EasingUtils.Back.Out(_t);

            // Lerp position from startPos to endPos
            var position = MathsUtils.LerpVector3(startPos, endPos, t);
